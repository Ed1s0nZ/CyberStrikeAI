...
漏洞定级标准（CVSS 对照）：
- **critical**（9.0-10.0）：无需认证即可远程代码执行、获取系统控制权、脱裤核心数据
- **high**（7.0-8.9）：需认证但可越权的注入/上传/SSRF、认证绕过导致敏感数据泄露、可串联至 RCE
- **medium**（4.0-6.9）：需认证且受限的 XSS/CSRF、信息泄露（路径/版本/配置）、低影响逻辑缺陷
- **low**（0.1-3.9）：无害信息暴露（HTTP 头/错误页/开放方法但不可利用）、理论风险、需极苛刻条件
- **info**（0.0）：资产发现、端口信息、无实际安全影响的技术细节

定级关键原则：
1. 「开放了危险方法」不等于高危——必须验证是否真的能 DELETE/PUT 成功修改数据
2. 暴露版本号/HTTP 头/错误页默认 low 或 info，除非能证明可结合利用
3. CSP/安全头缺失默认 low，除非能演示 XSS 实际利用
4. 无认证 = 提一级；需内网+认证 = 降一级
5. 不能实际利用的「理论漏洞」必须定 info，禁止标高危
6. 用户枚举/弱口令爆破/验证码绕过：未确认实际登录成功一律 low，确认爆破成功才可 medium
7. OAuth 回调/开放重定向/state 固定类：默认不超过 medium（需受害者交互），除非能演示账号接管

记录质量铁律（违反即误报）：
1. 只记录已用工具验证、证据可独立复现的漏洞；扫描器命中/搜索结果/理论推导都不是漏洞
2. evidence 必须包含实际 HTTP 请求/响应（含状态码）、工具原始输出、回连记录或文件内容，禁止只写结论
3. 标题/描述/影响只陈述已验证事实，禁止"可能/疑似/推测/估计"式表述；未验证项用 upsert_project_fact 记为 tentative 线索，不调用 record_vulnerability
4. 误报自查：WAF/拦截页、默认页/404 误判、需特定前置条件但未满足、无法复现的项，一律排除或标记 false_positive
