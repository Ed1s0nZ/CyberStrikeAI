...
const (
	factRhythmCore              = "记录会改变 scope、路由、认证状态、可利用性、风险或复现条件的**物质事实**：在下一安全检查点调用 `upsert_project_fact`（同 fact_key 覆盖更新），细粒度端口/URL/字符串观察批量合并。每验证出一条可复现漏洞（含 POC/影响）后调用 `record_vulnerability`；事实与漏洞可各记一次。上下文压缩、委派或收尾前刷新待写事实。原始凭据、token、flag、cookie 和密钥仅用受控 secret_ref、类型、长度、不可逆摘要与状态表示。未绑项目时在本轮保留结构化证据摘要。"
	factRhythmCoordinatorSuffix = "委派或子任务返回物质事实/漏洞时，由协调者合并、去重并写入。"
	factRhythmSubAgentSuffix    = "工具集中无上述工具时，在交付物末尾给出「待落库」结构化条目（fact_key 建议、summary、脱敏 body/POC 要点），供协调者合并写入。"
)
...
func factRecordingIncrementalRhythmBuiltin(coordinator, subAgent bool) string {
	var b strings.Builder
	b.WriteString("- **物质事实检查点**：记录会改变 scope、路由、认证状态、可利用性、风险或复现条件的事实。在下一安全检查点调用 ")
	b.WriteString(builtin.ToolUpsertProjectFact)
	b.WriteString("（同 fact_key 覆盖更新），细粒度观察批量合并。每验证出一条可复现漏洞（含 POC/影响）后调用 ")
	b.WriteString(builtin.ToolRecordVulnerability)
	b.WriteString("；事实与漏洞可各记一次。上下文压缩、委派或收尾前刷新待写事实。原始凭据、token、flag、cookie 和密钥仅用受控 secret_ref、类型、长度、不可逆摘要与状态表示。未绑项目时在本轮保留结构化证据摘要。")
	if coordinator {
		b.WriteString(factRhythmCoordinatorSuffix)
	}
	if subAgent {
...
- **敏感信息**：原始凭据、token、flag、cookie、API key 和私钥不进入事实、关系、artifact 名或命令；使用受控 ` + "`secret_ref`" + `、类型、长度、不可逆摘要、状态与必要非秘密标识。
- **category / fact_key 建议**：
  - 环境认知：` + "`target/`" + `、` + "`auth/`" + `、` + "`infra/`" + `、` + "`business/`" + `（body 用环境模板即可）
  - 发现与利用：` + "`finding/`" + `、` + "`chain/`" + `、` + "`exploit/`" + `、` + "`poc/`" + `（用攻击链模板记录：入口、逐步攻击链、已脱敏请求/响应或命令、证据、关联漏洞 ID）
...
func FactRecordingBlackboardSection(coordinatorDelegate bool) string {
	...
	b.WriteString(builtin.ToolUpsertProjectFact)
	b.WriteString("，fact_key 建议 `category/slug`（如 target/primary_domain），同 key 覆盖更新；body 记端口、版本、非秘密认证元数据与证据来源。\n")
	b.WriteString("- **发现与利用上下文**（审计复现）：fact_key 建议 finding/、chain/、exploit/、poc/ 前缀；body 记录完整攻击链（入口 → 步骤 → 已脱敏请求/响应或命令 → 现象 → 关联 related_vulnerability_id）；summary 写「什么 + 在哪 + 如何验证」一行要点。\n")
	...
	b.WriteString("- **敏感信息**：原始凭据、token、flag、cookie、API key 和私钥仅用受控 secret_ref、类型、长度、不可逆摘要和状态表示。\n")
...
func FactRecordingBlackboardSectionMarkdown(coordinatorDelegate bool) string {
	...
	b.WriteString("\n\n")
	b.WriteString("- **环境/目标/认证等认知**（非正式漏洞）：使用 **`upsert_project_fact`**，`fact_key` 建议 `category/slug`（如 `target/primary_domain`），同 key 覆盖更新；body 记端口、版本、非秘密认证元数据与证据来源。\n")
	b.WriteString("- **发现与利用上下文**（审计复现）：`fact_key` 建议 `finding/`、`chain/`、`exploit/`、`poc/` 前缀；body 记录完整攻击链（入口 → 步骤 → 已脱敏请求/响应或命令 → 现象 → 关联 `related_vulnerability_id`）；summary 写「什么 + 在哪 + 如何验证」一行要点。\n")
