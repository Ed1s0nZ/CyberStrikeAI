...
- 文件内相同“IP + 域名 + 端口 + 协议”（IP 与域名都缺失时为 Host）的后续行会标记为重复，不会提交。
...
资产以“IP + 域名 + 端口 + 协议”作为服务级去重依据；IP 与域名都缺失时才回退到 Host 去重。因此，同一域名下不同 IP 的资产（如测绘平台导出中 domain 为根域名、host 为子域名的数据）会被视为不同资产，同一主机的 `80/http` 和 `443/https` 也会被视为不同资产。
