...
function initWebshellTerminal(conn) {
    ...
    if (typeof term.attachCustomKeyEventHandler === 'function') {
        term.attachCustomKeyEventHandler(function (ev) {
            ...
            if ((ev.shiftKey && key === 'insert') || (accel && ev.shiftKey && key === 'v')) {
    ...
    function onTerminalContextMenu(e) {
        if (!readClipboardIntoTerminal()) return;
        e.preventDefault();
    }
...
function parseWebshellListItems(rawOutput) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (!/^[\-dlcbsp]/.test(trimmedLine) && parseWindowsDirEntry(trimmedLine) === null) continue;
        var name = '';
        var isDir = false;
        var size = '';
        var mode = '';
...
function saveWebshellConnection() {
    ...
    var body = { url: url, password: password, type: type, method: method === 'get' ? 'get' : 'post', protocol: protocol || 'classic', user_agent: headerText, encoding: encoding, os: osTag, remark: remark || url };
    if (typeof apiFetch === 'undefined') return;
